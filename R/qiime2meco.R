...
#' @param sample_data the sample metadata table, such as the sample-metadata.tsv.
#' @param taxonomy_data the taxonomy data, such as the taxonomy.qza.
...
#' }
#' @export
qiime2meco <- function(ASV_data, sample_data, taxonomy_data, phylo_tree = NULL, rep_fasta = NULL){
	if(!require(qiime2R)){
	...
	dataset <- microtable$new(sample_table = metadata, tax_table = taxa_table, otu_table = ASV, phylo_tree = phylo_tree, rep_fasta = rep_fasta)
	dataset
}
...
#' @param sample_data must be tab or comma seperated file, generally, a file with suffix "tsv" or "csv".
...
#' }
#' @export
qiime1meco <- function(otu_table, commented = TRUE, sample_data, phylo_tree = NULL, rep_fasta = NULL){
	if(!require(qiimer)){
	...
	if(grepl("csv", sample_data)){
		sample_info <- read.csv(sample_data, row.names = 1, stringsAsFactors = FALSE)
	}else{
		sample_info <- read.delim(sample_data, row.names = 1, stringsAsFactors = FALSE)
	}
